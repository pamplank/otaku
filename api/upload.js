...
import { json, getStore, isAdmin, validSlot, buildOf, uploadDir } from './_lib.js';
...
export async function POST(request) {
  ...
  if (!validSlot(build, slot)) return json({ error: 'Unknown slot' }, 400);
  ...
  const path = `${uploadDir(build, slot)}/${Date.now()}-${safe}`;
  try {
