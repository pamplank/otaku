...
export function hotspotDefs() {
  ...
  return [
    ...
    { id: 'logo', pos: [LG.width / 2 + 0.4, T.top + 0.3, L.logoZ], title: 'OPF logo sign',
      lines: [`Die-cut ${LG.board} board on the ${A.enabled ? 'front header' : 'front truss'}`, `≈ ${LG.width} m wide (est.)`,
        'Official CyberE logo only, as supplied'] },
    ...(A.enabled ? [{ id: 'arch', pos: [-L.archHalfWidth + 0.3, L.archHeaderBottom - 0.3, L.trussFrontZ + A.headerDepth / 2],
      title: A.box ? 'Header box' : 'Front header',
      lines: [`Printed teal header on ${A.box ? 'all four top beams' : 'the front beam'} · ${m(A.headerHeight)} deep`,
        A.box ? `≈ ${m(2 * L.archHalfWidth)} × ${m(L.trussFrontZ - L.trussBackZ + A.headerDepth)} on plan` : `≈ ${m(2 * L.archHalfWidth)} wide`,
        A.pillars ? `Front legs clad as ${A.pillarWidth} m pillars` : 'Truss legs left bare: clear sightlines',
        'Styled after the OPF entrance arch'] }] : []),
    ...
    { id: 'truss', pos: [-L.trussLegX, A.enabled ? 3.4 : T.top, L.trussFrontZ], title: 'Box truss',
      lines: [`Top of truss ${m(T.top)}`, `Ground-supported goalpost · ≈ ${T.spanOuter} m wide (est.)`,
        `${T.fixtures} fixtures under the front ${A.enabled ? 'header' : 'truss'}`, 'No ceiling rigging'] },
